...
describe("date-taken filter", () => {
	...
	it("does not collide with the month-scope from/to params", async () => {
		const cookie = await login();
		const tok = `dtc${Date.now()}`;
		const inBoth = await upload(cookie, `${tok}-both.png`);
		await setTakenAt(inBoth, "2004-05-15T10:00:00Z");

		// from/to (month scope) and dateFrom/dateTo (the filter facet) both
		// bound the same photo — they must compose (AND), not overwrite.
		const rows = await listPhotos(
			cookie,
			"?from=2004-05-01&to=2004-06-01&dateFrom=2004-05-01&dateTo=2004-05-31",
		);
		expect(rows.map((p) => p.id)).toContain(inBoth);

		// A dateFrom/dateTo range that excludes the photo, even though from/to
		// (month scope) would include it, correctly excludes it — proving the
		// two params are independent, composable conditions.
		const excluded = await listPhotos(
			cookie,
			"?from=2004-05-01&to=2004-06-01&dateFrom=2004-06-01&dateTo=2004-06-30",
		);
		expect(excluded.map((p) => p.id)).not.toContain(inBoth);
	});
});
