...
export async function batchProcessImages(
  ...
  const imageProcessingDetails = allImageFiles.reduce<ProcessImageDetail[]>((acc, image) => {
    const imgSettings = makeImageOutputMap(
      image,
      sourceDirectory,
      destDirectory,
      size,
  ...
  await Promise.all(
    imageProcessingDetails.map((detail) =>
      processOneImage(detail.srcImagePath, detail.outputImagePath, detail.options),
    ),
  )

  await Promise.all(
    allImageFiles.map((image) => copyInputImage(image, sourceDirectory, destDirectory)),
  )
}
...
function makeImageOutputMap(
  ...
  const imgBaseDirectories = format.reduce<Record<ImageFormat, string>>(
    (acc, format) => {
  ...
  const outputImages = Object.entries(imgBaseDirectories).flatMap(([format, baseDir]) =>
    size.map((size) => ({
...
async function copyInputImage(
  ...
  const destinationPath = path.join(
    destDirectory,
    sourceImageDetails.imageName,
    sourceImageDetails.imageStem,
  )
